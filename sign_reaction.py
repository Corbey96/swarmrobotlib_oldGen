        ...
        for sign_name, sign_pos, sign_distance in signs:
            if sign_name == "sign stop":
                if sign_distance <= 30 and not self.is_busy:
                    ...
                    if self.last_stop_time == datetime(2022, 1, 1, 22, 22, 22) or (current_time > stop_time_blocked):
                        print(
                            "follow line | detected stop sign - distance OK - STOP | distance = " + str(sign_distance))
                        ...
                        print("set last stop time to: " + str(last_stop_time))
                        time.sleep(3)
                        ...
                        print("follow line | too short time interval to last stop sign detection")
                else:
                    print("follow line | detected stop sign - distance to big | distance = " + str(sign_distance))
            if sign_name == "sign no entry":
                if sign_distance <= 30 and not self.is_busy:
                    ...
                    print(
                        "follow line | detected no entry sign - distance to big | distance = " + str(sign_distance))
